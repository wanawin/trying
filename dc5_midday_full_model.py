...
def parse_manual_filters_txt(raw_text: str):
    ...
    for raw_ln in lines:
        ...
        if not ln:
            if current:
                if current.get('name'): entries.append(current)
                else: skipped.append({'block': '\n'.join(raw_block)})
                current = None
                raw_block = []
            continue
        ...
        if low.startswith('type:'):
            if current is None: current = {'name': '', 'type': '', 'logic': '', 'action': ''}
            current['type'] = norm_ln.split(':', 1)[1].strip()
        elif low.startswith('logic:'):
            if current is None: current = {'name': '', 'type': '', 'logic': '', 'action': ''}
            after = norm_ln.split(':', 1)[1].strip()
            if after.lower().startswith('logic:'): after = after.split(':', 1)[1].strip()
            current['logic'] = after
...
def seed_sum_matches_condition(seed_sum: int, condition_str: str) -> bool:
    ...
    m = re.match(r'[≤<=]\s*(\d+)', s)
    if m: return seed_sum <= int(m.group(1))
...
# Debug sidebar initialization
st.sidebar.checkbox("Sidebar initialization reached.", value=True)
...
if trap_uploaded is not None:
    try:
        ...
        with open('dc5_trapv3_model.py', 'wb') as f: f.write(content)
...
if raw_manual:
    ...
    if st.sidebar.checkbox("Show normalized filter names for debugging"):
        ...
        for idx, pf in enumerate(parsed_entries):
            st.write(f"{idx}: '{pf['name']}' | Type: '{pf.get('type','')}' | Logic RAW: '{pf.get('logic','')}'")
else:
    st.write("No manual filters loaded.")

# ==============================
# Debug: Analyze each filter's logic detection
# ==============================
def detect_filter_pattern(pf):
    logic = pf.get('logic','') or ''
    logic_low = logic.lower()
    # detect sum range: look for patterns like '<n or >m' or 'sum <x or >y'
    m_sum = re.search(r'sum.*?<\s*(\d+).*?>\s*(\d+)', logic_low)
    if m_sum:
        return 'sum_range', {'low': int(m_sum.group(1)), 'high': int(m_sum.group(2))}
    # detect seed-sum conditional: 'between x and y if the seed sum is ...'
    m_keep = re.search(r'between\s*(\d+)\s*and\s*(\d+).*?if.*?seed sum', logic_low)
    if m_keep:
        return 'conditional_sum_if_seed', {'low': int(m_keep.group(1)), 'high': int(m_keep.group(2)), 'condition': logic}
    # detect seed contains conditional: 'seed contains X and winner contains ...'
    m_cond = re.search(r'seed contains\s*(\d+).*?contains', logic_low)
    if m_cond:
        seed_digit = int(m_cond.group(1))
        reqs = [int(x) for x in re.findall(r'(?:contain(?:s)?\s*)(\d)', logic_low)]
        return 'conditional_seed_contains', {'seed_digit': seed_digit, 'required': reqs}
    # other simple: if logic says 'sum = n'
    m_eq = re.match(r'sum\s*=\s*(\d+)', logic_low)
    if m_eq:
        return 'sum_equals', {'value': int(m_eq.group(1))}
    # unable to detect
    return 'unknown_logic', {'raw_logic': logic}

if parsed_entries and st.sidebar.checkbox("Show filter logic analysis chart"):
    analysis = []
    for idx, pf in enumerate(parsed_entries):
        pattern, params = detect_filter_pattern(pf)
        analysis.append({
            'index': idx,
            'name': pf.get('name',''),
            'raw_logic': pf.get('logic',''),
            'detected_pattern': pattern,
            'parameters': params
        })
    df_analysis = pd.DataFrame(analysis)
    st.sidebar.write("### Filter Logic Analysis")
    st.sidebar.dataframe(df_analysis)
...
# Apply filters and rest of app logic follows...
# ... (same as before)
